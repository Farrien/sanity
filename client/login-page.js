...
SN.LoginPage = {
	...
	Signup : function() {
		...
		if (rL.V() === '' || rL.V().length < 10) {
			SN.ShowTooltip('Укажите номер телефона. Например, 88005553535 или +78005553535.', rL);
			rL.FocusOn();
			return false;
		}
		var rN = mr.Dom('#SignForm input[name=regName]');
		if (rN.V() === '' || rN.V().length < 2) {
			SN.ShowTooltip('Укажите свое имя.', rN);
			rN.FocusOn();
			return false;
		}
		t.NewClass('loading');
		mr.SendForm('../get/Userdata/Signup', '#SignForm', function(response) {
			...
			if (r.result == false) {
				t.SetTxt('Ошибка');
				SN.ShowTooltip('Указанный Вами номер телефона уже занят. Если это Ваш телефон, то войдите со своим паролем или восстановите его.', t);
				mr.Timers.CreateTimer(1.0, function() {
					t.SetTxt('Зарегистрироваться');
				});
			}
			if (r.result == true) {
				SN.ShowTooltip('Вы были успешно зарегистрированы. Дождитесь SMS с вашим паролем. <br><a class="SimpleLink" href="/login/?ownp=origin">Вернуться на страницу входа</a>', t);
				rL.V('');
				rN.V('');
				ToggleFormFrames();
			}
	...
	Signin : function() {
		...
		if (rN.V() === '') {
			SN.ShowTooltip('Заполните это поле.', rN);
			rN.FocusOn();
			return false;
		}
		t.NewClass('loading');
	...
	RestorePassword : function() {
		var rL = mr.Dom('#AuthForm input[name=authLogin]');
		if (rL.V() === '' || rL.V().length < 10) {
			SN.ShowTooltip('Укажите свой телефон для получения пароля.', rL);
			rL.FocusOn();
			return false;
		}
