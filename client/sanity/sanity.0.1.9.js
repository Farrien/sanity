...
var Miracle = {
	...
	GetJSON : function(url, query, callback, $long) {
		var querylist = '';
		for (var key in query) {
			querylist += key + "=" + encodeURIComponent(query[key]) + '&';
		}
		if (DEBUG) console.log('Creating a query...\nTarget URL: '+url+'\nRequest Script: '+querylist.substring(0, querylist.length - 1)+'\nContent-Type: application/json');
		return this.jsonGetReq(url, querylist.substring(0, querylist.length - 1), callback);
	},
	HttpGet : function() {
		var t1 = [],
			t2 = [],
	...
	jsonGetReq : function(url, dataForm, callback){
		var reqLoc = url || '';
		var xtp = this.createRequest();
